...
export default function ProducerPanel() {
  ...
  const count = simConfig?.producers || 2;

  return (
    <div className="glass-card p-5 h-full shadow-lg overflow-hidden flex flex-col">
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2.5">
          <div className="w-2.5 h-2.5 rounded-full bg-[var(--accent-cyan)]" style={{ boxShadow: '0 0 10px var(--accent-cyan)' }} />
          <span className="text-[12px] font-black text-white uppercase tracking-[0.1em]">Producers ({count})</span>
        </div>
      </div>

      <div className="space-y-2.5 flex-1 overflow-y-auto pr-1">
        {Array.from({ length: count }, (_, i) => {
          const name = `Producer-${i + 1}`;
